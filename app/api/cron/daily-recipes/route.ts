...
export async function GET(request:Request){
 ...
 try{
  const contactsResponse=await resend("/contacts"); if(!contactsResponse.ok){reportError("daily-recipes.contacts","Could not load contacts",{status:contactsResponse.status});return NextResponse.json({error:"Could not load subscribers."},{status:502});}
  ...
  reportInfo("daily-recipes.complete",{subscribers:contacts.length,sent,failed});
  return NextResponse.json({ok:failed===0,subscribers:contacts.length,sent,failed,recipes:picks.map(r=>r.title)});
 }catch(error){
  reportError("daily-recipes.network",error);
