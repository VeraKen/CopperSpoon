...
function friendlyAuthMessage(error:unknown){
 ...
 if(/email rate limit exceeded/i.test(message))return "Too many confirmation emails were requested. Your account may already exist, so try logging in. Otherwise, please wait before trying again.";
 if(/email not confirmed/i.test(message))return "Please confirm your email address before logging in. Check your inbox and spam folder.";
 if(/invalid login credentials/i.test(message))return "That email address or password is incorrect.";
 if(/user already registered/i.test(message))return "An account already exists for this email address. Please log in instead.";
...
export default function AuthForm({mode}:{mode:"login"|"signup"}){
 ...
 async function submit(event:FormEvent<HTMLFormElement>){
  ...
  catch(error){setStatus("error");setMessage(friendlyAuthMessage(error));}
 }
 async function google(){setStatus("loading");setMessage("");try{const result=await supabase().auth.signInWithOAuth({provider:"google",options:{redirectTo:`${window.location.origin}/account`}});if(result.error)throw result.error;}catch(error){setStatus("error");setMessage(friendlyAuthMessage(error));}}
